...
export class ProductPage {
  ...
  constructor() {
    // The code and the language are the only two inputs. Reloading on a language
    // change is not optional: the name, the description, the attribute values
    // and the variant labels are all resolved by the server in the requested
    // culture, so a switch that only retranslated the chrome would leave Spanish
    // product copy under English labels.
    effect(() => {
      const code = this.code();
      const culture = this.culture.active();

      untracked(() => this.load(code, culture));
    });
  }

  private load(code: string, culture: string): void {
    if (!code) {
    ...
    this.products.get(code, culture).subscribe({
      next: (product) => {
        this.state.set({ status: 'done', product });
        this.selection.set(initialSelection(product));
        this.shown.set(null);
        this.canonicalise(product);
        this.declare(product);
      },
      error: (error: { status?: number }) => {
        ...
        this.seo.clear();
        this.state.set({ status: error.status === 404 ? 'notFound' : 'failed' });
      },
    });
  }

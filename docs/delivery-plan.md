...
- **Current phase:** 0 — CI and OpenAPI done (`P0-1`…`P0-5`); 11 tasks left
- **Next task:** `P0-6` + `P0-7`, migrations and Testcontainers — they land together
...
- [ ] `P0-6` EF Core Design + an initial migration reproducing today's schema exactly; `MigrateAsync` replaces `EnsureCreatedAsync` — **M**
- [ ] `P0-7` Testcontainers + a reset helper; the first integration test drains the outbox end to end — **M**
- [ ] `P0-8` `TimeProvider` injected into `Product` and `Order` — **S**
- [ ] `P0-9` `SearchEval --suite` switch, search suite behind it — **S**
- [ ] `P0-10` `TelemetrySources` constants replacing the four string literals — **S**

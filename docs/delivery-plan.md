...
- **Current phase:** 7 — **accounts, audit, and the issuer swapped**. Sign-in is Authorization Code + PKCE against whichever issuer the API names, so Keycloak shows its own login page and the shop never handles a credential. 494 tests, format gate clean, and the same 6 backoffice specs green against BOTH issuers (3 consecutive runs each) plus 14 storefront specs.
- **Next task:** the storefront's look (`docs/delivery-plan.md` backlog, "A storefront that looks like a shop"), or `P7-9` — agent `client_credentials` and Token Exchange, which is the half a fake issuer could not honestly provide and which phase 11 needs.

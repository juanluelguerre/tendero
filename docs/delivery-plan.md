...
- **Current phase:** **a style sweep in a documentation commit left the catalogue unable to be read, and only Docker could see it.** Seven aggregates keep their collections in private fields and seven EF configurations name those fields BY STRING; `5864494` took the leading underscore off the fields and left the strings alone. Four complex collections then threw on model build and three plain properties went quietly to shadow state, so the API could not have served a product — with `-warnaserror`, `dotnet format`, the architecture rules and 500 domain tests all green, because none of them can see inside a string. The only suite that failed is the one that needs a container, on the machine where `docker pull` had been broken for days. Seven strings repaired, `tests/Persistence.Tests` added as the guard that needs **no** database, verified by mutation both ways. **And the imaging half of `SeedImages` never worked on Linux at all**: SkiaSharp ships the Windows and macOS binaries with the wrapper and the Linux one is opt-in, so four tests failed with `libSkiaSharp` missing on the platform CI runs on. **567 tests, 547 green, 20 skipped with a reason, format gate clean.** Before that: **the catalogue is being drawn, and the generator that draws it is not the one I wrote.** `tools/SeedImages` grew a port, `IIllustrationGenerator`, with two keyed adapters: SDXL inside the .NET process on ONNX Runtime with DirectML, and one HTTPS call to `gemini-2.5-flash-image` — nano banana, the model the first eight illustrations were made with by hand. **The hosted one drew the catalogue, for quality and not for speed**: SDXL's output beside those eight is flat, grey and soft at the edges, a white product disappears against the background, and six prompt rewrites did not close the gap because the gap is the model. 18 of 100 images exist, at ~6 s and ~$0.03 each — **~$3 for the hundred** — with the vision pass reporting 18 usable and two notes, one of them a real catalogue error. The swap cost one thirty-line interface and did not touch the deterministic tail. **The returns promise landed on the product page too**, which closes the last non-scale row of the storefront backlog: the window is published at `GET /api/returns/policy` instead of being typed into a template, because fourteen days was already written twice with nothing comparing the two. Before that: **the blog caught up with the code.** The four diagrams in article 00 were a photograph of 2026-09-02 and had gone quietly wrong: two bounded contexts where there are five, UCP labelled phase 3 when it is 9 and 11, Qdrant labelled phase 2 when it is 8, a shipped agent surface (WebMCP) missing entirely, and **Grafana drawn as if it ran when the AppHost does not declare it** — the exact failure the article condemns two paragraphs above it. All four regenerated from their `.html` sources through `.svg` to 2000px `.png`, the article's own table corrected (6 endpoints → 33 routes, 117 tests → 503 + 37 + 20, 13 ADRs → 21) and the prose rewritten in plainer English. Articles 16 and 17 went from broken links out of `blog/index.md` to finished drafts in the same day — the two pieces the queue had been holding that were blocked on nothing — and then **17 folded into 16 as its closing section**, because both defects were found by building the same product detail page and that is a better piece than two. Outline 09 stopped carrying the pre-100-product baseline it would have published as a "before". Before that: a review pass for SOLID/DRY across back and front, **merged** to `develop` as PR [#2](https://github.com/juanluelguerre/tendero/pull/2) — 503 backend tests, 37 frontend and 20 browser specs green (one honest skip), every `.md` audited against the code. Since then, an evening of environment failures on Linux under Rider, none of them in the code and all three in the notebook: two dotnet roots, nvm's PATH, and a failed `docker pull` that deletes the tag it cannot replace. Before that: the storefront's look, off the board's backlog and ahead of phase 8 — **the home page is a shop now**: departments off the taxonomy phase 2 built, offers off the promotion engine, novedades off `available_from`, and a category page that browses. 495 tests, format gate clean. Merged to `develop` as PR [#1](https://github.com/juanluelguerre/tendero/pull/1) — opened as a pull request, fast-forwarded on the author's say-so, and GitHub recorded it as merged either way.
- **Next task:** the remaining **82 illustrations**, which are one command and about nine minutes (`generate --take 82`, then `verify`) — **on the machine that has the key and the network**, since `GEMINI_API_KEY` is read from the environment and from nowhere else. `B13BAG0710`'s colour is **done** (2026-09-08): the seed says `negro`, the picture was not touched, and the gate did not move — es 0.813 / 0.633, en 0.766 / 0.556, against a throwaway Elasticsearch rather than the stack's. Doing it first was deliberate: the prompt reads the colour out of the catalogue, so drawing before fixing would have drawn a grey sleeve twice. Then `--provider sdxl` is worth one more honest look now that the prompt no longer carries measurements. `P7-9`'s Token Exchange still waits for phase 11.
...
**Decisions taken 2026-09-08** — 1 · **a closed option does not describe a colour, it picks the nearest one**, and which one is nearest is measured rather than eyeballed. `B13BAG0710`'s felt is RGB (68, 66, 62) — a warm charcoal at L* ≈ 28, neither ink nor the mid grey the word suggests — so of the two candidates the catalogue actually offers, `negro` wins by distance. Saying it that way is a smaller claim than "the sleeve is black", and it is the one the data supports. The gate was run rather than reasoned about, because colour is indexed and the margin is ~0.013. · 2 · **a member EF is told about by string is a reference the compiler cannot check**, so it gets a test that builds the model and refuses a property in shadow state — and that test lives where it needs no database, because the eighteen failures that found this one all needed Docker and Docker is what had been broken. · 3 · **the fix is the strings and not the fields**: `.editorconfig` asks for no underscore and `this.`, the sweep was right, and renaming the domain back to satisfy a persistence detail would be the tail wagging the aggregate. · 4 · `SkiaSharp.NativeAssets.Linux` is added unconditionally rather than behind an OS condition — a restore that differs per machine is the defect article 06 is about, and the package is nothing but `.so` files a Windows build never copies.

**Decisions taken 2026-09-07** — 1 · **the illustration generator is a port with two adapters**, and the one that draws the hundred is the hosted one: a catalogue with two visual registers is not a catalogue, and the eight that already existed set the register · 2 · **the local SDXL adapter stays**, because it is the only path with no key, no network and no bill, and the only one that reproduces an image from its `item_id` — `IsDeterministic` is declared on the port rather than left for somebody to discover · 3 · **the port is at the level of a BATCH**, since SDXL opens a five gigabyte session once per run and a per-image port would have made it open ninety-two · 4 · **measurements are stripped from the prompt by code**, after three rounds of asking the model not to letter `4 mm` onto a midsole: a figure beside a part reads as that part's label, and it was never earning its place in a drawing · 5 · **a vision note never fails a batch** — a model arguing about olive cannot throw away work that is fine — and a colour mismatch is repaired in the catalogue rather than by redrawing · 6 · the article-17 diagram was **redrawn as the architecture** rather than as SDXL's insides, because a diagram of one implementation would have drawn the half that did not draw the hundred.
